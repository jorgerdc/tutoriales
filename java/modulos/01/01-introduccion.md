...
* La primera versión de Java incluida en el llamado JDK (Java Development Kit) fue lanzada
  en 1996 (JDK 1.0).
* Oracle adquiere Sun Microsystems en 2010. En 2011 se libera el JDK 7.
* En 2014 se libera el JDK 8, una de las versiones más populares y ampliamente adoptadas
  por la gran cantidad de características nuevas que incluye.
* A partir de Java 11, Oracle libera versiones LTS (Long-Term Support) cada 3 años. JDK 11
  (2018) y JDK 17 (2022) son versiones LTS.

### 1.1.2 La tecnología Java y grupos de productos

Java puede ser considerado no solo como un lenguaje de programación, sino como una
tecnología completa que abarca varios grupos de productos:
...
* **Java SE (Standard Edition)**: Es la versión estándar de Java, que incluye las
  ...
  estándar.
* **Jakarta EE, antes Java EE (Enterprise Edition)**: Es una extensión de Java SE que
  incluye herramientas y APIs para el desarrollo de aplicaciones empresariales que corren
  en grandes servidores distribuidos.
* **Java ME (Micro Edition)**: Es una versión reducida de Java destinada a dispositivos
  móviles y sistemas embebidos con recursos limitados.
...
* El JDK se puede descargar desde el [sitio oficial de
  Oracle](https://www.oracle.com/java/technologies/downloads) o utilizar distribuciones
...
* Son un conjunto de reglas y recomendaciones para escribir código Java de manera
  consistente y legible.
* Algunas convenciones importantes incluyen:
  * Nombres de clases en mayúscula inicial (CamelCase)
  * Nombres de métodos y variables en minúscula inicial (camelCase)
  * Uso adecuado de espacios y sangrías para mejorar la legibilidad
  * Comentarios claros y concisos para explicar el propósito del código
* Seguir estas convenciones facilita la colaboración entre desarrolladores y mejora la
...
Estos temas son fundamentales para interactuar con el usuario a través de la consola, solo
se mencionan para tener una idea básica de su funcionamiento del programa anterior, se
...
que proporcionan funcionalidades predefinidas para facilitar el desarrollo de aplicaciones
en Java. El API de Java está organizado en paquetes, que son grupos lógicos de clases
relacionadas. Algunos paquetes comunes en el API de Java incluyen:
...
* `java.io`: Incluye clases para manejo de entrada y salida, como archivos y flujos de
  datos.
* `java.net`: Contiene clases para programación de redes y comunicación a través de
  sockets.
* El API de Java esta totalmente documentado y se puede consultar en línea en el sitio
  oficial de Oracle: [Java SE API Documentation](https://docs.oracle.com/en/java/javase/).
  Este formato de documentación es conocido como Javadoc y puede generarse automáticamente
  a partir del código fuente utilizando la herramienta `javadoc`.

### 1.7.2 Paquetes en Java

* Los paquetes en Java representan una forma de organizar y agrupar clases relacionadas.
...
* Cuando una clase  hace uso de otra que no se encuentra en su propio paquete, es
  necesario importar la clase o el paquete correspondiente utilizando la palabra clave
  `import`. Ejemplo:
...
* El ejemplo anterior importa la clase `ArrayList` del paquete `java.util` para utilizarla
  en la clase `MiClase`. Otra forma de importar todas las clases de un paquete es
  utilizando el asterisco `*` por ejemplo, `import java.util.*;`
...
* Las buenas prácticas relacionadas con el concepto de paquetes recomiendan:
  ...
  * Seguir una convención basada en el dominio invertido de la organización (por ejemplo,
  `com.ejemplo.proyecto`).
  * No hacer uso de clases sin definir paquetes, ya que se colocan en el paquete por
    defecto llamado `default package`, lo que puede generar conflictos de nombres y
...
* La variable de entorno `CLASSPATH` en Java es una configuración que indica a la JVM
  dónde buscar las clases y paquetes necesarios para compilar y para ejecutar una
  aplicación Java.
* Puede incluir rutas a directorios o archivos JAR que contienen las clases y paquetes
  requeridos.
* Un archivo JAR (Java ARchive) es un formato de archivo que agrupa múltiples archivos
  relacionados, como clases y recursos, en un solo archivo comprimido.
* Para compilar o ejecutar un programa, el compilador y la JVM respectivamente buscan las
  clases en los siguientes lugares:
...
* Para definir el valor de la variable `CLASSPATH`,  se puede realizar de 2 formas:

    1. Temporalmente en la línea de comandos al ejecutar el programa:
    ...
    2. Permanentemente en el sistema operativo, no recomendada y poco utilizada en sistemas
     grandes y modernos. Ejemplo en Linux/Mac:
...
* Java proporciona herramientas de línea de comandos para compilar y ejecutar aplicaciones
  Java. En aplicaciones profesionales, es común utilizar IDEs que integran estas
...
#### Compilación a línea de comandos
...
## 1.9 Prácticas propuestas <img src="img/book.png" alt="Ejercicios" width="50" height="50">
...
### 1.9.2 Hola mundo con Java a línea de comandos
...
### 1.9.3 Crear y ejecutar una aplicación Java a línea de comandos

* Crear una aplicación simple que haga uso de varias clases.
* Definir una clase principal con el método `main`.
* Crear  una clase adicional en el mismo paquete que la clase principal.
* Crear otra clase en un paquete diferente.
* Crear una clase que no tenga paquetes definidos (default package).
* Crear un archivo JAR que incluya una tercera clase en un paquete diferente.
* Todas las clases anteriores deben ser utilizadas desde la clase principal.
* Compilar todas las clases utilizando el comando `javac`, asegurándose de incluir las
  rutas necesarias en la variable `CLASSPATH`.
* Ejecutar la aplicación utilizando el comando `java`, asegurándose de incluir las rutas
  necesarias en la variable `CLASSPATH`, y verificar la salida.
* Un escenario posible de la estructura de archivos es el siguiente:
  ...
  * El archivo JAR puede llamarse `libreria.jar` y contener una clase adicional en un
    paquete diferente, por ejemplo, `com.ejemplo.libreria.OtraClase`.
  * Un  caso de estudio  podría ser una aplicación de mensajes que utilice otras clases
    para imprimir saludos en diferentes idiomas. Proponer clases como `SaludoEspañol`,
    `SaludoIngles`, etc.

### 1.9.4 Interactuar con JShell
